...
import com.awake.CloserLink.project.Common.Enums.VailDateTypeEnum;
...
import com.awake.CloserLink.project.Entitys.LinkGotoDO;
import com.awake.CloserLink.project.Mapper.*;
import com.awake.CloserLink.project.Mq.producer.ShortLinkStatsSaveProducer;
import com.awake.CloserLink.project.Service.ShortLinkService;
import com.awake.CloserLink.project.Utils.HashUtil;
import com.awake.CloserLink.project.Utils.LinkUtil;
import com.awake.CloserLink.project.Utils.SslUtils;
...
//            }
    ...
    public void updateShortLink(ShortLinkUpdateReqDTO shortLinkUpdateReqDTO) {
        ...
        if (!Objects.equals(hasLinkDO.getValidDateType(), shortLinkUpdateReqDTO.getValidDateType())
            ...
            //不相等删除关联表数据
            stringRedisTemplate.delete(String.format(RedisKeyConstant.GOTO_SHORT_LINK_KEY, shortLinkUpdateReqDTO.getFullShortUrl()));
            ...
            if (hasLinkDO.getValidDate() != null && hasLinkDO.getValidDate().before(currentDate)) {
                if (Objects.equals(shortLinkUpdateReqDTO.getValidDateType(), VailDateTypeEnum.PERMANENT.getType()) || shortLinkUpdateReqDTO.getValidDate().after(currentDate)) {
                    stringRedisTemplate.delete(String.format(RedisKeyConstant.GOTO_IS_NOTNULL_SHORT_LINK_KEY, shortLinkUpdateReqDTO.getFullShortUrl()));
                }
            }
        }
    }
    ...
    public void restoreUrl(String shortLink, ServletRequest request, ServletResponse response) throws IOException {
        ...
        if (StrUtil.isBlank(originalLink)) {
            ...
            try {
                ...
                if (linkGotoDO == null) {
                    //短链接不存在设置空对象
                    stringRedisTemplate.opsForValue().set(String.format(RedisKeyConstant.GOTO_IS_NOTNULL_SHORT_LINK_KEY, fullShortLink), "-");
                    ((HttpServletResponse) response).sendRedirect("/page/notfound");
                    //封控
                }
                LambdaQueryWrapper<LinkDO> queryWrapper = Wrappers.lambdaQuery(LinkDO.class)
...
//                        .eq(LinkDO::getGid, linkGotoDO.getGid());
    ...
    public void shortLinkStats(String fullShortUrl, String gid, ShortLinkStatsRecordDTO statsRecord) {
        statsRecord.setCurrentDate(new Date());
